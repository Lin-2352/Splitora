...
export default function BalanceChartCard() {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-slate-900 light:bg-white border border-slate-700 light:border-slate-200 p-3 rounded-xl shadow-xl light:shadow-md z-50">
          <p className="text-white light:text-slate-900 font-bold mb-1 tracking-tight">{groupName}</p>
          <p className={isOwed ? "text-app-cyan" : "text-app-purple"}>
  ...
  return (
    <div className="bg-slate-900/80 light:bg-white backdrop-blur-xl border border-slate-800 light:border-slate-200 rounded-3xl p-6 shadow-xl light:shadow-sm flex flex-col h-full min-h-[350px] hover:-translate-y-2 hover:shadow-2xl light:hover:shadow-lg transition-all duration-300">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-lg font-semibold text-white light:text-slate-900 tracking-tight">Net Balances</h2>
      ...
      <div className="flex-1 relative flex items-center justify-center">
        ...
        {!loading && chartData.length > 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <span className={`text-lg font-bold ${netBalance >= 0 ? 'text-app-cyan' : 'text-app-purple'}`}>
      ...
      {!loading && chartData.length > 0 && (
         <div className="mt-4 flex justify-between border-t border-slate-800 pt-4">
           <div>
             ...
             <p className="text-app-cyan font-bold">₹{(totalOwed || 0).toFixed(2)}</p>
           </div>
           <div className="text-right">
             <p className="text-xs text-slate-500">You pay</p>
             <p className="text-app-purple font-bold">₹{(totalDebt || 0).toFixed(2)}</p>
           </div>
         </div>
      )}
    </div>
