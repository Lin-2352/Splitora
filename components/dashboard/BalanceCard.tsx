...
export default function BalanceCard() {
  ...
  return (
    <div className="bg-app-card border border-app-border rounded-2xl p-6 shadow-sm flex flex-col h-full">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-white flex items-center gap-2">
          <span className="material-symbols-outlined text-app-cyan text-[20px]">timeline</span>
          Recent Activity
        </h2>
      </div>
              ...
              >
                ...
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${
                  isSettle 
                    ...
                    : 'bg-[#a82ee2]/10 text-[#a82ee2]'
                }`}>
                  <span className="material-symbols-outlined text-[16px]">
                    {isSettle ? 'handshake' : 'receipt_long'}
                  </span>
                </div>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[10px] text-slate-600">{getTimeAgo(activity.createdAt)}</span>
                    <span className="text-[10px] text-[#00f2fe]/60 font-medium truncate">{activity.groupName}</span>
                  </div>
                </div>
              </li>
            );
