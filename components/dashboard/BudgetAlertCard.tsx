...
export default function BudgetAlertCard() {
  ...
  const barColor = isOver ? '#ef4444' : percentage > 75 ? '#eab308' : '#00f2fe';
  ...
  return (
    <div className="bg-app-card border border-app-border rounded-2xl p-5 shadow-sm relative overflow-hidden">
      ...
      {isOver && (
        <div className="absolute -top-10 -right-10 w-32 h-32 bg-red-500 rounded-full mix-blend-screen filter blur-[80px] opacity-20 animate-pulse"></div>
      )}
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {budget !== null && (
          <button onClick={clearBudget} className="text-[10px] text-slate-600 hover:text-slate-400 transition-colors cursor-pointer">Reset</button>
        )}
      </div>
      ...
      {loading ? (
          ...
          {editing ? (
            <div className="space-y-2">
              <input
                ...
                className="w-full bg-slate-800/50 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-600 outline-none focus:border-app-cyan transition-colors"
                autoFocus
            ...
            <div className="text-center py-2">
              ...
              <button
                ...
                className="w-full py-2.5 bg-gradient-to-r from-[#00f2fe]/20 to-[#a82ee2]/20 border border-slate-700 text-white text-xs font-bold rounded-xl hover:border-app-cyan/50 transition-all cursor-pointer"
              >
                + Set Budget
              </button>
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center">
            <span className={`text-xs font-bold ${isOver ? 'text-red-400' : 'text-slate-300'}`}>
