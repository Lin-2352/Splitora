...
export default function SpendingChart({ groupId, title = 'Spending Trends' }: SpendingChartProps) {
  ...
  return (
    <div className="bg-slate-900/80 light:bg-white backdrop-blur-xl border border-slate-800 light:border-slate-200 rounded-3xl p-6 shadow-xl light:shadow-sm flex flex-col hover:-translate-y-2 hover:shadow-2xl light:hover:shadow-lg transition-all duration-300">
      {/* Title row */}
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-white light:text-slate-900 tracking-tight">{title}</h2>
        {/* Range Selector */}
        <div className="flex gap-1 bg-slate-900/50 light:bg-slate-100/50 rounded-lg p-1 border border-slate-800 light:border-slate-200">
          {rangeOptions.map(opt => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all cursor-pointer ${
                range === opt.id
                  ? 'bg-gradient-to-r from-[#00f2fe] to-[#4facfe] text-slate-900 shadow-[0_0_10px_rgba(0,242,254,0.4)]'
