...
export default function ChatBot() {
  ...
  /* ── Track mobile viewport (reactive) ──────────────────────────── */
  useEffect(() => {
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-indigo-600 light:bg-[var(--accent-purple)] hover:bg-indigo-500 light:hover:bg-purple-600 text-white shadow-xl flex items-center justify-center transition-all duration-200"
      ...
      >
        {isOpen ? (
          ...
          <>
            ...
            {!isOnline && (
              <span className="absolute top-0 right-0 w-3 h-3 bg-red-500 rounded-full border-2 border-[#1a1a2e] light:border-white" />
            )}
          </>
        )}
      </button>
      ...
      {isOpen && (
        <>
          ...
          />
          <div
            className="fixed z-50 flex flex-col shadow-2xl border overflow-hidden
            ...
            style={{
              ...
              animation: isMobile ? "chatSlideUp 0.25s ease-out" : "chatFadeIn 0.2s ease-out",
            }}
          >
          ...
          {/* Header */}
          <div className="px-4 py-3 bg-indigo-600 light:bg-[var(--accent-purple)] flex items-center justify-between shrink-0">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-indigo-100 light:text-purple-100 text-xs mt-0.5">Ask about your groups, bills, or balances</p>
            </div>
            <div className="flex items-center gap-1 shrink-0">
              ...
              {confirmClear ? (
                ...
                >
                  <svg className="w-4 h-4 text-indigo-100 light:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {showHistory ? (
            <div className="flex-1 overflow-y-auto flex flex-col">
              <div className="p-3 border-b border-white/10 light:border-slate-200 flex items-center justify-between">
                <span className="text-xs font-semibold text-gray-400 light:text-slate-500 uppercase tracking-wider">Chat History</span>
                <button
                  onClick={() => { setShowHistory(false); setHistorySearch(""); }}
                  className="text-xs text-indigo-400 light:text-purple-600 hover:text-indigo-300 light:hover:text-purple-500 transition-colors"
                >
                  Back
                </button>
              </div>
              ...
              {sessions.length > 3 && (
                <div className="px-3 pt-2">
                  <input
                    ...
                    className="w-full bg-white/5 light:bg-slate-100 border border-white/10 light:border-slate-200 rounded-lg text-xs text-gray-300 light:text-slate-700 placeholder-gray-600 light:placeholder-slate-400 px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-indigo-500 light:focus:ring-purple-500"
                  />
                </div>
              )}
              {filteredSessions.length === 0 ? (
                <div className="flex flex-col items-center justify-center flex-1 text-gray-500 light:text-slate-400 py-8">
                  <p className="text-sm">{historySearch ? "No matching chats" : "No chats yet"}</p>
                </div>
              ) : (
                <div className="divide-y divide-white/5 light:divide-slate-100 flex-1 overflow-y-auto">
                  {filteredSessions.map((s) => (
                    <div
                      key={s.id}
                      className={`flex items-center gap-2 px-3 py-2.5 cursor-pointer transition-colors group ${
                        s.id === activeId ? "bg-indigo-600/20 light:bg-purple-50" : "hover:bg-white/5 light:hover:bg-slate-50"
                      }`}
                      onClick={() => switchSession(s.id)}
                    >
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm truncate ${s.id === activeId ? "text-indigo-300 light:text-purple-700 font-medium" : "text-gray-300 light:text-slate-700"}`}>
                          {s.title}
                        </p>
                        <p className="text-[10px] text-gray-500 light:text-slate-400 mt-0.5">
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
                {messages.length === 0 && (
                  <div className="text-center mt-6">
                    ...
                    <p className="text-gray-100 light:text-slate-800 text-sm">Hi! I am your Splitora assistant.</p>
                    <p className="text-gray-400 light:text-slate-500 text-xs mt-1">Try one of these:</p>
                    <div className="mt-3 flex flex-wrap gap-2 justify-center">
                      {quickPrompts.map((q) => (
                        <button
                          ...
                          className="text-xs bg-white/8 light:bg-purple-50 hover:bg-white/15 light:hover:bg-purple-100 text-indigo-200 light:text-purple-800 font-medium px-3 py-1.5 rounded-full border border-indigo-400/30 light:border-purple-300 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                        >
                          {q}
                        </button>
                      ))}
                    </div>
                    <p className="text-gray-600 light:text-slate-400 text-[10px] mt-4 hidden sm:block">
                      Press <kbd className="px-1 py-0.5 bg-white/10 light:bg-slate-200 rounded text-[10px]">Ctrl+Shift+K</kbd> to toggle · <kbd className="px-1 py-0.5 bg-white/10 light:bg-slate-200 rounded text-[10px]">Esc</kbd> to close
                    </p>
                  </div>
                )}
                    ...
                    <div className="max-w-[82%] group/msg">
                      <div
                        className={`px-3 py-2 rounded-2xl text-sm leading-relaxed ${
                          msg.role === "user"
                            ? "bg-indigo-600 light:bg-[var(--accent-purple)] text-white rounded-br-sm"
                            : "bg-white/8 light:bg-slate-100 text-gray-100 light:text-slate-800 rounded-bl-sm border border-white/10 light:border-slate-200"
                        }`}
                      >
                        {msg.role === "assistant" ? (
                          <div
                      ...
                      <div className={`flex items-center gap-2 mt-0.5 px-1 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                        <span className="text-[10px] text-gray-600 light:text-slate-400">{formatTime(msg.timestamp)}</span>
                        {msg.role === "assistant" && (
                          <button
                            onClick={() => copyMessage(msg.content, i)}
                            className="opacity-0 group-hover/msg:opacity-100 text-[10px] text-gray-500 light:text-slate-400 hover:text-indigo-400 light:hover:text-purple-600 transition-all flex items-center gap-0.5"
                            title="Copy message"
                          >
                            {copiedIdx === i ? (
                              <>
                ...
                {loading && (
                  <div className="flex justify-start">
                    <div className="bg-white/8 light:bg-slate-100 border border-white/10 light:border-slate-200 px-4 py-2.5 rounded-2xl rounded-bl-sm">
                      <span className="flex gap-1 items-center">
                        {[0, 150, 300].map((d) => (
                          <span
                            key={d}
                            className="w-1.5 h-1.5 bg-indigo-400 light:bg-purple-500 rounded-full animate-bounce"
                            style={{ animationDelay: `${d}ms` }}
                          />
                        ))}
                        <span className="text-[11px] text-indigo-300/70 light:text-purple-500 ml-2">Thinking…</span>
                        <button
              ...
              {/* Input bar */}
              <div className="border-t border-white/10 light:border-slate-200 p-3 flex flex-col gap-1 bg-[#1a1a2e] light:bg-white shrink-0">
                <div className="flex gap-2 items-end">
                  <textarea
                    ...
                    disabled={loading || !isOnline}
                    className="flex-1 resize-none bg-white/5 light:bg-slate-50 border border-white/10 light:border-slate-300 rounded-xl text-sm text-gray-100 light:text-slate-900 placeholder-gray-400 light:placeholder-slate-500 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 light:focus:ring-purple-500 disabled:opacity-50"
                    style={{ maxHeight: "100px", overflowY: "auto" }}
                  />
                  <button
                    onClick={() => sendMessage()}
                    disabled={!input.trim() || loading || !isOnline}
                    className="w-9 h-9 rounded-xl bg-indigo-600 light:bg-[var(--accent-purple)] hover:bg-indigo-500 light:hover:bg-purple-600 text-white flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed transition-colors shrink-0"
                    title="Send message"
                  >
                ...
                <div className="flex justify-between items-center px-0.5">
                  <span className="text-[10px] text-gray-600 light:text-slate-400 hidden sm:inline">
                    Shift+Enter for new line
                  </span>
                  <span className={`text-[10px] tabular-nums ${input.length > MAX_INPUT_LENGTH * 0.9 ? "text-red-400" : input.length > MAX_INPUT_LENGTH * 0.75 ? "text-yellow-500" : "text-gray-600 light:text-slate-400"}`}>
