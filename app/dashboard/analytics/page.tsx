...
export default function AnalyticsPage() {
  ...
  const BarTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-slate-900 light:bg-white border border-slate-700 light:border-slate-200 p-3 rounded-xl shadow-xl light:shadow-md text-sm">
          <p className="text-white light:text-slate-900 font-bold mb-1 tracking-tight">{label}</p>
  ...
  const PieTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-slate-900 light:bg-white border border-slate-700 light:border-slate-200 p-3 rounded-xl shadow-xl light:shadow-md text-sm">
          <p className="text-white light:text-slate-900 font-bold tracking-tight">{payload[0].name}</p>
          <p className="text-slate-300 light:text-slate-600">₹{payload[0].value.toFixed(0)}</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 md:p-8">
          <div className="max-w-[1600px] mx-auto space-y-6">
            ...
            {/* Header */}
            <div>
              <h1 className="text-2xl font-bold text-white light:text-slate-900 tracking-tight">Analytics</h1>
              <p className="text-sm text-slate-500 mt-1">Compare spending and credit across all your groups.</p>
            </div>

            {/* Summary Stats */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="bg-slate-900/80 light:bg-white backdrop-blur-xl border border-slate-800 light:border-slate-200 shadow-xl light:shadow-sm rounded-3xl p-5 hover:-translate-y-1 transition-all">
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Groups</p>
                <p className="text-xl font-bold text-white light:text-slate-900 mt-1 tracking-tight">{groupCount}</p>
              </div>
              <div className="bg-slate-900/80 light:bg-white backdrop-blur-xl border border-slate-800 light:border-slate-200 shadow-xl light:shadow-sm rounded-3xl p-5 hover:-translate-y-1 transition-all">
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Total Credit</p>
                <p className="text-xl font-bold text-[#00f2fe] mt-1 tracking-tight">₹{totalCredit.toFixed(0)}</p>
              </div>
              <div className="bg-slate-900/80 light:bg-white backdrop-blur-xl border border-slate-800 light:border-slate-200 shadow-xl light:shadow-sm rounded-3xl p-5 hover:-translate-y-1 transition-all">
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Total Spending</p>
                <p className="text-xl font-bold text-[#a82ee2] mt-1 tracking-tight">₹{totalSpending.toFixed(0)}</p>
              </div>
              <div className="bg-slate-900/80 light:bg-white backdrop-blur-xl border border-slate-800 light:border-slate-200 shadow-xl light:shadow-sm rounded-3xl p-5 hover:-translate-y-1 transition-all">
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Net Balance</p>
                <p className={`text-xl font-bold mt-1 tracking-tight ${net >= 0 ? 'text-[#00f2fe]' : 'text-[#a82ee2]'}`}>
                  {net >= 0 ? '+' : ''}₹{net.toFixed(0)}
                </p>
              </div>
            </div>
            ...
            <div className="bg-slate-900/80 light:bg-white backdrop-blur-xl border border-slate-800 light:border-slate-200 shadow-xl light:shadow-sm rounded-3xl p-6 hover:-translate-y-2 transition-all">
              <h2 className="text-lg font-semibold text-white light:text-slate-900 tracking-tight mb-1">Group Comparison</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-slate-900/80 light:bg-white backdrop-blur-xl border border-slate-800 light:border-slate-200 shadow-xl light:shadow-sm rounded-3xl p-6 hover:-translate-y-2 transition-all">
                <h2 className="text-lg font-semibold text-white light:text-slate-900 tracking-tight mb-1">Activity Share</h2>
              ...
              <div className="bg-slate-900/80 light:bg-white backdrop-blur-xl border border-slate-800 light:border-slate-200 shadow-xl light:shadow-sm rounded-3xl p-6 hover:-translate-y-2 transition-all">
                <h2 className="text-lg font-semibold text-white light:text-slate-900 tracking-tight mb-1">Per-Group Balances</h2>
                ...
                <ul className="space-y-2">
                  {groupData.length === 0 ? (
                    ...
                    groupData.map((g, i) => (
                      <li key={i} className="flex items-center justify-between p-3 rounded-xl bg-slate-800/30 light:bg-slate-100/50 border border-slate-800 light:border-slate-200">
                        <span className="text-sm text-white light:text-slate-900 font-bold tracking-tight truncate">{g.groupName}</span>
